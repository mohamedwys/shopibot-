...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const rateLimitResponse = rateLimit(request, RateLimitPresets.GENEROUS, {
    useShop: true,
    namespace: '/apps/widget-settings/loader',
  });
  ...
  try {
    // Extract shop domain from request headers (app proxy adds this)
...
// This route is called via Shopify App Proxy from the storefront
export const action = async ({ request }: ActionFunctionArgs) => {
  const routeLogger = createLogger({ route: '/apps/widget-settings' });
  ...
  // App proxy requests come from Shopify's infrastructure, not from the storefront origin
  // So we don't validate origin for app proxy requests
  routeLogger.debug('Processing app proxy request');
  ...
  const rateLimitResponse = rateLimit(request, RateLimitPresets.MODERATE, {
    useShop: true,
    namespace: '/apps/widget-settings/action',
  });
  ...
  try {
    ...
    routeLogger.info({ shop: shopDomain }, 'Processing chat request via app proxy');

    if (!shopDomain) {
      routeLogger.warn('No shop domain found in app proxy request');
      return json(
    ...
    logError(error, "Chat API Error (App Proxy)");
    return json({
